...
int main(int argc, char * argv[])
{
	...
	itemType* SetC_gpu; cudaMalloc((void**)&SetC_gpu, d_par*(subseq_count + numimputevalue)*l_par*sizeof(itemType));
    itemType* Q_gpu; cudaMalloc((void**)&Q_gpu, d_par*l_par*sizeof(itemType));
    itemType* LB_gpu; cudaMalloc((void**)&LB_gpu, d_par*(subseq_count + numimputevalue)*num_lb*sizeof(itemType));
	
	for (int i = 0; i < numimputevalue; i++)
	{
		itemType s = impute(S, R, i, SetC_gpu, Q_gpu, LB_gpu);
		S_init[i] = S[L_par + i];
		S_predicted[i] = s;
